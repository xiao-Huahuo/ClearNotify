# TODO
### 客户端UI
- [ ] 将主页-时事热点栏移动到中央文件栏相邻的左边,和中央文件栏并排.
- [ ] 主页-最近解析栏在未滑出时应该隐藏.
- [ ] 解决header的搜索框下的标签显示为{"name":"政策","value":90}的问题
- [ ] sidebar的图标模式,应该让所有图标竖直居中排列,且sidebar呈现胶囊形,然后header的最左就可以延伸到页面的最左边.
- [ ] 将主页+搜索页(正中间补充一个)+数据分析页+智能体页+会话历史+我的收藏+办事进度中心+个人中心+系统设置+管理员页面 的标题都改成主页中间"看见政策"的样式,且被全局调控以便于明暗切换和颜色切换.
- [ ] 将主页的文件上传区的三个按钮的形状从胶囊变成半径相等的圆形.半径以最长的为主.
### 人力完成(UI)
- [ ] 莫兰迪色系数据统计的引入.
- [ ] 响应式布局的实现,争取在移动端适配.
